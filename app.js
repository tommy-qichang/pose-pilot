...
class PosePilotApp {
    constructor() {
        ...
        // API Configuration - Users need to add their API keys
        this.config = {
            openaiApiKey: '', // Users need to add their OpenAI API key
            geminiApiKey: '', // Users need to add their Gemini API key
            chatgptModel: 'gpt-4-vision-preview'
        };
    ...
    async getChatGPTSuggestions() {
        const prompt = `You are a professional photography coach. Analyze this photo and provide exactly 5 specific, actionable suggestions to improve the composition, pose, and overall quality. 
...
Be specific, practical, and focus on improvements that would make a noticeable difference in the photo quality.`;
    ...
    async showDemoSuggestions() {
        ...
        this.suggestions = [
            ...
            {
                title: "Dynamic Pose Ideas",
                cameraView: "Try shooting from a slightly higher angle for a flattering perspective",
                interaction: "Add movement - walking, jumping, or interacting with props",
                lighting: "Use side lighting to create depth and dimension",
                background: "Include leading lines that guide the eye to your subject"
            },
            {
                title: "Composition Enhancement",
                cameraView: "Apply rule of thirds - place subject at intersection points",
                interaction: "Create layers by including foreground, middle, and background elements",
                lighting: "Balance exposure between subject and background",
                background: "Use contrasting colors to make your subject pop"
            },
            {
                title: "Professional Finishing",
                cameraView: "Ensure your camera is level and straight",
                interaction: "Capture genuine expressions and natural gestures",
                lighting: "Check for even lighting across the subject's face",
                background: "Remove distracting elements from the frame"
            }
        ];
        ...
        for (let i = 0; i < 2; i++) {
            try {
                ...
                if (data.candidates && data.candidates[0] && data.candidates[0].content) {
                    // Handle the generated image (this would need to be adapted based on actual API response)
                    this.displayGeneratedPhoto(i, data.candidates[0].content);
                } else {
